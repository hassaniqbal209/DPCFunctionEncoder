...
initial_state_bounds = np.array([[0.01, 0.01], [1.0, 1.0]])
init_state = initial_state_bounds[0] + (initial_state_bounds[1] - initial_state_bounds[0]) * np.random.rand(2)


plot = False

def run_mpc_simulation(c1, c2, N, dt, Ulim, Q, R, T_sim, ref_traj, init_state):
    ...
    def dynamic(x_state, u):
        ...
        dx1 = c1 * (1.0 - v) * p - c2 * casadi.sqrt(x1) 
        dx2 = c1 * v * p + c2 * casadi.sqrt(x1) - c2 * casadi.sqrt(x2)  

        
        dx1 = casadi.if_else(x1 + dx1 > 1.0, 0, dx1)
        dx2 = casadi.if_else(x2 + dx2 > 1.0, 0, dx2)
        dx1 = casadi.if_else(x1 + dx1 < 0.0, 0, dx1)
        dx2 = casadi.if_else(x2 + dx2 < 0.0, 0, dx2)
    ...
    # Input Constraints
    opti.subject_to(opti.bounded(0,U,Ulim));
    ...
    u0 = np.zeros((2,N));
    next_states = np.zeros((2,N+1)) #np.tile(init_state.reshape(-1, 1), (1, N+1))
